const episodes = [
    {
        id:1,
        season: 1,
        name: "Welcome to the Playground",
        air_date: 2021-11-06,
        length: 43,
        synopsis: "Orphaned sisters Vi adn Powder bring trouble to Zaun's underground streets in the wake of a heist in posh Piltover.",
        characters: [
            "Jayce",
            "Grayson",
            "Vander",
            "Young Ekko",
            "Marcus",
            "Powder",
            "Jules",
            "Deckard",
            "Huck",
            "Mylo",
            "Verne",
            "Claggor",
            "Silco",
            "Vi",
            "Benzo",
            "Singed"
        ]
    },
    {
        id: 2,
        season: 1,
        name: "Some Mysteries Are Better Left Unsolved",
        air_date: 2021-11-06,
        length: 41,
        synopsis: "Idealistic inventor Jayce attempts to harness magic through science -- despite his mentor's warning. Criminal kingpin Silco tests a powerfull substance.",
        characters: [
            "Jayce",
            "Grayson",
            "Vander",
            "Young Ekko",
            "Marcus",
            "Powder",
            "Deckard",
            "Huck",
            "Mylo",
            "Claggor",
            "Silco",
            "Vi",
            "Ximena",
            "Young Caitlyn",
            "Mel Medarda",
            "Shoola",
            "Bolobok"
        ]
    },
    {
        id: 3,
        season:1,
        name: "The Base Violence Necessary for Change",
        air_date: 2021-11-06,
        length: 44,
        synopsis: "An epic showdown between old rivals results in a fateful moment for Zaun. Jayce and Viktor risk it all for their research.",
        characters: [
            "Jayce",
            "Grayson",
            "Vander",
            "Young Ekko",
            "Marcus",
            "Powder",
            "Deckard",
            "Huck",
            "Mylo",
            "Claggor",
            "Silco",
            "Vi",
            "Mel Medarda",
            "Benzo",
            "Heimerdinger"
        ]
    },
    {
        id: 4,
        season: 1,
        name: "Happy Progress Day!",
        air_date: 2021-11-06,
        length: 40,
        synopsis: "With Piltover prospering from their tech, Jayce and Viktor weigh their next move. A familiar face re-emerges from Zaun to wreak havoc.",
        characters: [
            "Jayce",
            "Grayson",
            "Vander",
            "Caitlyn",
            "Bolobok",
            "Powder",
            "Jinx",
            "Burly Henchman",
            "Elora",
            "Salo",
            "Silco",
            "Mrs. Kiramman",
            "Mel Medarda",
            "Sevika",
            "Heimerdinger",
            "Shoola",
            "Vi",
            "Firelight Leader"
        ]
    },
    {
        id: 5,
        season: 1,
        name: "Everybody Wants to Be My Enemy",
        air_date: 2021-11-06,
        length: 40,
        synopsis: "Rogue enforcer Caitlyn tours the undercity alongside Vi to track down Silco. Jayce puts a target on his back trying to root Piltover corruption.",
        characters: [
            "Grayson",
            "Jayce",
            "Sky",
            "Babette",
            "Young Caitlyn",
            "Marcus",
            "Caitlyn",
            "Elora",
            "Viktor",
            "Warden",
            "Mel Medarda",
            "Jinx",
            "Amara",
            "Claggor",
            "Silco",
            "Vi",
            "Sevika"
        ]
    },
    {
        id: 6,
        season: 1,
        name: "When These Walls Come Tumbling Down",
        air_date: 2021-11-06,
        length: 42,
        synopsis: "An eager protege undermines his mentor on the council as a magical tech rapidly evolves. With authories in pursuit, Jinx must face her past.",
        characters: [
            "Jayce",
            "Vander",
            "Young Viktor",
            "Marcus",
            "Powder",
            "Jinx",
            "Vi",
            "Mel Medarda",
            "Huck",
            "Caitlyn",
            "Viktor",
            "Firelight Leader",
            "Silco",
            "Sevika",
            "Heimidinger",
            "Singed",
            "Theiram"
        ]
    },
    {
        id: 7,
        season: 1,
        name: "The Boy Savior",
        air_date: 2021-11-06,
        length: 40,
        synopsis: "Caitlyn and Vi meet an ally in Zaun's streets and head into a frenzied battle with a common foe. Viktor makes a dire decision.",
        characters: [
            "Jayce",
            "Marcus",
            "Powder",
            "Caitlyn",
            "Elora",
            "Viktor",
            "Mylo",
            "Renni",
            "Mel Medarda",
            "Finn",
            "Jinx",
            "Silco",
            "Ekko",
            "Vi",
            "Singed"
        ]
    },
    {
        id: 8,
        season: 1,
        name: "Oil and Water",
        air_date: 2021-11-06,
        length: 40,
        synopsis:"Disowned heir Mel and her visiting mother trade combat tactics. Caitlyn and Vi forge an unlikely alliance. Jinx undergoes a startling change.",
        characters: [
            "Jinx",
            "Silco",
            "Ekko",
            "Vi",
            "Mel Medarda",
            "Singed",
            "Mrs. Kiramman",
            "Mr. Kiramman",
            "Sky",
            "Elora",
            "Young Mel",
            "Salo",
            "Hoskel",
            "Bolbok",
            "Viktor",
            "Jayce",
            "Caitlyn",
            "Ambessa Medarda"
        ]
    },
    {
        id: 9,
        season: 1,
        name: "The Monster You Created",
        air_date: 2021-11-06,
        length: 39,
        synopsis: "Perilously close to war, the leaders of Piltover and Zaun reach an ultimatum. But a fateful standoff changes both cities forever.",
        characters: [
            "Caitlyn",
            "Ambessa Medarda",
            "Ekko",
            "Vi",
            "Mrs. Kiramman",
            "Mylo",
            "Jinx",
            "Mel Medarda",
            "Viktor",
            "Claggor",
            "Silco",
            "Hoskel",
            "Shoola",
            "Sky",
            "Jayce",
            "Vander",
            "Salo",
            "Sevika"
        ]
    }
]

module.exports = episodes